package org.distributedea.input.batches.evcharging.advnnhh;
...
public class BatchHomoMethodsEVChargingAdvnnhh implements IInputBatch {

	public Batch batch() throws IOException {
		
		Batch batch = new Batch();
		batch.setBatchID("homoMethodsEVChargingAdvnnhh");
		batch.setDescription("Porovnání homogenních modelů : EVChargingAdvnnhh");
		
		Job jobI = InputEVCharging.test02();
		jobI.getIslandModelConfiguration().setIndividualDistribution(true);
		jobI.setPlanner(new PlannerInitialisationOneMethodPerCore());
		...
		ProblemEVCharging problem = (ProblemEVCharging) jobI.getProblem();
		DatasetDescription datasetDesr = (DatasetDescription) jobI.getDatasetDescription();
		
		PreProcessingEVChargingRunServers pp1 = new PreProcessingEVChargingRunServers(
				problem.getPort(), 16, datasetDesr.exportDatasetFile());
		batch.addPreProcessings(pp1);
		...
		String YLABEL0 = "fitness jako rozptyl odběru el. energie";
		PostProcessing psMat0 = new PostProcBoxplot(YLABEL0);
		
		String YLABEL1 = "fitness jako rozptyl odběru el. energie";
